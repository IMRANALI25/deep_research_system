...
# (name="Tavily_Deep_Web_Search", description="Get Web Search Results from Tavily")


@function_tool
async def tavily_deep_search(query: str) -> List[dict]:
    try:
        """Search Tavily with the given query (agent may rewrite it)."""
        ...
        #         tavily_client_agent.py - tavily_deep_search()""")
        #     exit(1)
...
def create_tavily_client_agent(file_format: HC.FileFormat, research_topic: str) -> Agent:
    
    # Initialize  Variables
    if research_topic is not None:
        TAVILY_QUERY = research_topic
    else:
        raise ValueError("""\n❌ TAVILY_QUERY Variable is not defined.
        Please set the TAVILY_QUERY variable before using this function.
        tavily_client_agent.py - create_tavily_client_agent()""")
        exit(1)
    ...
    INSTRUCTIONS = """You are the Expert Deep Research Assistant.You always
        ...
        Always call tavily_deep_search() using the user’s query exactly, without rewriting it.
        ...
        plan a summarized query for deep research using content extraction 
        method of tavily extract(). You also use tavily web search for 
    ...
    try:
        ...
        tavily_client_agent: Agent = Agent(
            name="tavily_client_agent",
            model=LP.LLM_MODEL,
            instructions=f"{INSTRUCTIONS}\n\n{SPECIAL_INSTRUCTIONS}",
            tools=[tavily_deep_search],
            model_settings=MODEL_SETTINGS,
        )
        if tavily_client_agent is None:
            print("🤖 Tavily Client Agent could not be created.")
            exit(1)
        else:
            return tavily_client_agent
